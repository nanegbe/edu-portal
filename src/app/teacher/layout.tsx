...
import { Menu, X, LogOut, BookOpen, ClipboardList, FileText, Award, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function TeacherLayout({
    ...
    const navItems = [
        ...
        { name: 'Scores', href: '/teacher/scores', icon: Award },
    ];
                    ...
                    <div className={`flex items-center gap-3 bg-slate-700 rounded-lg transition-all ${isCollapsed ? 'p-1 justify-center' : 'p-3'}`}>
                        ...
                        {!isCollapsed && (
                            <div className="flex-1 min-w-0 animate-fadeIn text-left">
                                <p className="text-sm font-medium truncate">{teacherInfo.name}</p>
                                <p className="text-xs text-slate-400 truncate">{teacherInfo.subject}</p>
                            </div>
                        )}
                    </div>
