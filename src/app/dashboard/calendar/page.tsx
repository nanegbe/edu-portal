...
export default function AcademicCalendar() {
    ...
    const handleAddYear = async () => {
        if (newYear.year && newYear.term1Start && newYear.term1End) {
            const terms = [
                { name: newYear.type === 'semester' ? 'First Semester' : 'First Term', startDate: newYear.term1Start, endDate: newYear.term1End },
                { name: newYear.type === 'semester' ? 'Second Semester' : 'Second Term', startDate: newYear.term2Start, endDate: newYear.term2End }
            ];

            if (newYear.type === 'trimester' && newYear.term3Start) {
                terms.push({ name: 'Third Term', startDate: newYear.term3Start, endDate: newYear.term3End });
            }
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    alert(err.error || 'Failed to add academic year');
                }
            } catch (error) {
    ...
    const handleUpdateSemester = async () => {
        if (selectedSemester) {
            try {
                const res = await fetch(`/api/calendar?id=${selectedSemester.id}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        type: 'term',
                ...
                alert('An error occurred while updating semester');
            }
        }
    };

    const handleSetActiveSemester = async (termId: string) => {
        try {
            const res = await fetch(`/api/calendar?id=${termId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ type: 'term', status: 'ACTIVE' })
            });
            ...
            if (res.ok) {
                ...
                const err = await res.json();
                alert(err.error || 'Failed to set active semester');
            }
        } catch (error) {
            alert('An error occurred while setting active semester');
        }
    };
    ...
    const handleDeleteYear = async (yearId: string) => {
        if (confirm('Are you sure you want to delete this academic year? This will remove all associated data.')) {
            try {
                const res = await fetch(`/api/calendar?id=${yearId}`, {
                    method: 'DELETE'
                });
                ...
                alert('An error occurred while deleting year');
            }
        }
    };
    ...
    const updateTermStatus = async (termId: string, newStatus: string) => {
        try {
            const res = await fetch(`/api/calendar?id=${termId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ type: 'term', status: newStatus })
            });
            ...
            if (res.ok) {
                ...
                const err = await res.json();
                alert(err.error || 'Failed to update term status');
            }
        } catch (error) {
